import { searchArchiveBatches, detailsResultSchema } from "./archive-discovery.js";
...
export function createDiscoveryTools(React) {
  ...
  function useSessionDetails(sessions, call) {
      ...
      const load = async () => {
        ...
        for (let start = 0; start < ids.length && active; start += 20) {
          ...
          try {
            ...
            items.push(...batch.map(sessionId => found.get(sessionId) ?? { sessionId, turnCount: null, path: null, error: "宿主未返回会话详情" }));
          } catch (error) {
