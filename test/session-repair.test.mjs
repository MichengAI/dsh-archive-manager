...
test('仅转换已知自动化消息来源，保留正文和归属且不修改输入',()=>{
 ...
 assert.equal(result.count,1);assert.equal(rows[1].data.source.kind,'automation');
 assert.equal(result.rows[1].data.source.plugin,'dsh-automation');assert.deepEqual(JSON.parse(result.rows[1].data.source.summary),source);
...
const fakeFormat = {
 currentVersion:3,
 createRestore(header) { const events=[]; return { decodeRow:row=>events.push(row), finish:()=>({header:{...header,version:3},events,inheritedEventCount:0}) }; },
 encodeHeader:header=>header, encodeEvent:event=>event
};
test('修复令牌绑定原文件，原文件保留且禁止覆盖已有代际',async()=>{
 ...
 try {
  ...
  await assert.rejects(plan.publish('0'.repeat(64),async()=>{}),/变化/);
  await writeFile(source,original+'\n','utf8');
  await assert.rejects(plan.publish(plan.token,async()=>{}),/变化/);
  await writeFile(source,original,'utf8');
  await assert.rejects(plan.publish(plan.token,async()=>{throw new Error('会话已打开');}),/打开/);
  await plan.publish(plan.token,async()=>{});
  assert.equal(await readFile(source,'utf8'),original);
  assert.match(await readFile(target,'utf8'),/dsh-automation/);
  await assert.rejects(plan.publish(plan.token,async()=>{}),{code:'EEXIST'});
 } finally { await rm(directory,{recursive:true,force:true}); }
});

import { createSessionHealthPanel } from '../src/session-health-ui.js';
test('异常默认折叠，先诊断再确认修复，修复后刷新详情',async()=>{
 const slots=[];let cursor=0,diagnoses=0,repairs=0,retries=0;
 ...
 const props={items:[{sessionId:'a',error:'cannot safely transform unclassified message source'}],sessions:[{id:'a',title:'旧天气会话'}],diagnoseSession:async()=>{diagnoses++;return {repairable:true,token:'a'.repeat(64),reason:'可修复',advice:'保留旧日志'};},repairSession:async input=>{repairs++;assert.equal(input.token,'a'.repeat(64));return {repaired:true};},retry:()=>retries++};
 const render=()=>{cursor=0;return Panel(props);};
 const flat=n=>!n||typeof n!=='object'?[]:[n,...(n.children??[]).flat(Infinity).flatMap(flat)];
 let tree=render();assert.ok(flat(tree).filter(n=>n.type==='details').every(n=>!n.props.open));
 const button=(tree,label)=>flat(tree).find(n=>n.type==='button'&&n.children.includes(label));
 assert.equal(button(tree,'确认修复'),undefined);
 await button(tree,'诊断会话').props.onClick();tree=render();assert.equal(diagnoses,1);assert.equal(repairs,0);
 await button(tree,'确认修复').props.onClick();assert.equal(repairs,1);assert.equal(retries,1);
});
