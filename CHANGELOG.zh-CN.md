...
- 诊断无法自动修复时保留缺失、权限及损坏分类；详情提示改为渲染时翻译，避免语言函数变化触发重复读取。清理已移除全部操作的界面死词条，并补充预览打开时切换页签的页面回归。

- 修复收藏在索引未知时误清理、切换页签后预览复开、Unicode 搜索偏移及代理对截断；诊断覆盖缺摘要的归档 ID，不再以错误文案决定工件校验入口，修复文件采用私有权限。补齐远程接口、写锁、继承计数与发布失败回归，并同步双语文档。
- 正文检索尚无持久化索引，每批最多 20 条、批内逐条读取；不能中止在途批次。大量或超长日志建议先缩小筛选范围，尚无规模或时延承诺。

- 修复快速预览 Esc 连带关闭设置、筛选后分组计数、未归档筛选空态及复制失败翻译；完成独立真实宿主端到端回归。

- 统一选中、复选框、焦点、警告、更新按钮与高亮配色；补齐诊断修复、预览及错误提示中英文，更新入口跟随当前界面语言。

- 诊断修复区采用紧凑状态卡片、统一线性图标与主次按钮，精简重复提示并优化窄屏布局。

- 新增异常会话诊断与定向修复：默认收起详细错误，按原因给出建议；旧自动化来源通过宿主校验后可确认生成新版日志，原日志保留。

- 快速预览加宽并支持 Markdown 表格、代码块与角色区分，可切换原文高亮；复制会话 ID 和路径补齐图标。

- 新增会话轮次显示、轮次升降序和未知值置后；归档与未归档行均可复制会话 ID、会话路径，详情读取失败支持重试。

- 搜索范围切换常驻输入框左侧，日期浮层位于右侧；移除下方更多筛选区域，清除日期独立于搜索范围。

- 新增归档正文分批检索、命中片段与高亮、项目／收藏／更新时间组合筛选，以及更多菜单中的只读快速预览；失败支持重试，切换搜索条件忽略旧响应。

- 归档设置页改为紧凑分组列表：标题点击打开、收藏下拉、右侧星标和快捷图标；项目支持折叠，恢复打开与删除收进更多菜单。

- 新增宿主持久化收藏和「只看收藏」，归档与恢复后保留收藏标记。
- 新增按项目、标题和闲置天数预览归档候选，默认保护收藏与活跃会话，确认前可逐条取消。
- 批量归档、恢复和删除增加进度、部分完成明细及剩余项重试；支持在本次管理页打开期间撤回最近一批成功归档。

## 0.1.44 - 2026-09-18

- 设置里点「打开会话」不再误报「宿主未保留」。
- 支持 DeepSeek Harness `0.1.6-alpha.2`，并继续兼容更早版本。归档页、打开归档和永久删除在官方接口变更后仍可用。

## 0.1.43 - 2026-09-16

- 支持 DeepSeek Harness 0.1.6，并继续兼容更早版本。设置里只保留本插件的归档页，侧栏归档菜单在 0.1.6 上也能用。
- 打开归档会话或「恢复并打开」会回到原来的工作区和分组，不再被立刻清掉。
- 批量归档和恢复改为逐条处理：中途失败会保留已完成的部分，找不到的会话不再报错打断。

## 0.1.42 - 2026-09-14

- 新增「已归档 / 未归档」页签，默认展示已归档；支持跨项目选择会话批量归档，也可通过项目或未分组菜单归档该组全部聊天。
- 统一会话操作文案，提升批量归档按钮与菜单的辨识度；归档失败时保留确认框并显示错误，可直接重试，同时清理过期提示。
- 新增 Typert `workspaceRegistry/archiveSessions` 接口，接受 `{ sessionIds: string[] }` 批量归档跨项目会话，支持去重与幂等，无效会话导致整批拒绝。

## 0.1.41 - 2026-09-14

- 优化归档会话的批量操作栏：选择摘要与操作分组显示，修复窄内容区内按钮文字折行的问题。
- 未选择时简化操作栏；跨筛选选择的数量和影响范围独立显示，避免遗漏不可见的已选会话。
