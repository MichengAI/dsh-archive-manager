...
- 支持 DeepSeek Harness `0.1.7-alpha.1`，并继续兼容此前已支持的宿主。
- 官方会话列表提供菜单插槽时保留该列表，只在菜单中追加删除会话。更早的宿主仍使用本插件的会话列表。
- 归档运行中的会话前会先确认停止。若宿主记录了置顶，归档时一并取消。
- 旧自动化来源修复跟随宿主当前日志代际，包括第四代会话日志。
- 设置页的按钮、输入框、页签、下拉、菜单、勾选和对话框改用 Ant Design。排序、项目、查找范围和收藏筛选都是下拉框。半选勾选使用同一套勾选框。
- 归档列表面板使用自己的可访问名称，项目和会话菜单会报出是否展开。闲置天数清空后不再显示成 1。
- 打包时压缩设置页客户端脚本，并在文件末尾保留 Ant Design 和 dayjs 的许可说明。
- 批量操作进行时页签变淡且不能切换。搜索框只保留一层边框。旧宿主的图标按钮不再缺少 key。
- 设置页遇到运行中的会话时直接确认停止并归档，不再指到侧栏。会话修复把父会话的直属子会话证据传给格式目录。
