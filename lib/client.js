      ...
      return runSessionBatch(ids, async (id) => {
        ...
        if (kind === "archive") {
          ...
          if (options.idleDays !== void 0) {
            ...
            if (!idleArchiveCandidate(latest.byId[id], { days: options.idleDays, favorites, currentId: currentSessionId2(latest), pending: options.getPending?.() ?? options.pending })) return false;
          }
          await archive(id);
        ...
        const viewConversation = async (session, restore = false) => {
          ...
          try {
            await openArchivedConversation({
              prepare: restore ? async (id) => {
                await focusSessionWorkspace?.(id);
              } : void 0,
              restore: async (id) => {
      ...
      function factsFromSessionStatus(status) {
        ...
        if (status instanceof Map) {
          for (const [id, entry] of status) {
            if (entry?.pendingInteraction !== void 0) pending.set(id, entry.pendingInteraction);
      ...
      function ArchivedSessionMenu({ busy, title, t, onRestoreOpen, onDelete }) {
        ...
        return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
          ...
          onSelect: (id) => {
            setOpen(false);
            if (busy) return;
            if (id === "restoreOpen") return onRestoreOpen();
