window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* 单个删除按钮的目标：先子代理（含多层），再本会话。fork 分支不是 subagent，不会被收进来。
		*/
		function collectSessionAndDescendantIds(sessionId, byId) {
			if (typeof sessionId !== "string" || sessionId.length === 0) return [];
			const catalog = byId ?? {};
			const childrenByParent = /* @__PURE__ */ new Map();
			for (const session of Object.values(catalog)) {
				if (session === void 0 || session.origin !== "subagent" || typeof session.id !== "string" || session.id.length === 0) continue;
				const parentId = session.parentSessionId ?? session.parentSession;
		...
		/**
		* 归档设置页“删除全部”的目标：全部已归档 id，加上这些父会话下仍在目录中的 subagent 子会话。
		* 每棵树都是先子后父，避免级联删除后再删子会话时被当成未知。
		*/
		function collectArchivedDeleteAllIds(archivedSessionIds, byId) {
			const catalog = byId ?? {};
			const ids = [];
			const seenArchived = /* @__PURE__ */ new Set();
			for (const id of archivedSessionIds ?? []) {
				if (typeof id !== "string" || id.length === 0 || seenArchived.has(id)) continue;
				seenArchived.add(id);
				ids.push(id);
			}
			const result = [];
			const seen = /* @__PURE__ */ new Set();
			const addTree = (id) => {
				for (const item of collectSessionAndDescendantIds(id, catalog)) {
			...
			for (const id of ids) {
				if (catalog[id]?.origin === "subagent") continue;
				addTree(id);
			}
			for (const id of ids) {
		...
		*/
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, unarchiveSession, deleteSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, t }) {
			...
			const confirmDeleteSession = () => {
				...
				const ids = collectSessionAndDescendantIds(rootId, sessionSnapshot.byId);
				(async () => {
					for (const id of ids) {
						try {
							...
							if (id !== rootId && /no such session/.test(message)) continue;
							throw reason;
						}
					}
				})().then(() => {
		...
		function ArchivedSessionsSection({ sessionStore, workspaceStore, unarchiveSession, deleteSession, t }) {
			...
			const groups = (0, react.useMemo)(() => {
				// 列表仍隐藏 subagent；“删除全部”另用 collectArchivedDeleteAllIds 收父会话和子会话。
			...
			const deleteAllSessionIds = (0, react.useMemo)(() => collectArchivedDeleteAllIds(workspaceState.archivedSessionIds, sessions.byId), [workspaceState.archivedSessionIds, sessions]);
			...
			const confirmDelete = async () => {
				...
				setError(null);
				try {
					for (const id of deleteTarget.all ? deleteAllSessionIds : collectSessionAndDescendantIds(deleteTarget.id, sessions.byId)) {
						try {
							await deleteSession(id);
						} catch (reason) {
							const message = reason instanceof Error ? reason.message : String(reason);
							if ((deleteTarget.all || id !== deleteTarget.id) && /no such session/.test(message)) continue;
							throw reason;
						}
					}
					setDeleteTarget(null);
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [(0, react_jsx_runtime.jsx)("style", { children: ARCHIVE_SETTINGS_CSS }), (0, react_jsx_runtime.jsxs)("header", { className: "dsham_settingsHeader", children: [(0, react_jsx_runtime.jsxs)("div", { children: [(0, react_jsx_runtime.jsx)("h2", { children: t("archives.title") }), (0, react_jsx_runtime.jsx)("p", { className: "dsham_settingsIntro", children: t("archives.description") })] }), (0, react_jsx_runtime.jsxs)("button", { type: "button", className: "dsham_settingsDanger", disabled: busy || deleteAllSessionIds.length === 0, onClick: () => setDeleteTarget({ all: true }), children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconTrashOutline16, {}), t("archives.deleteAll")] })] }), (0, react_jsx_runtime.jsxs)("div", { className: "dsham_settingsToolbar", children: [(0, react_jsx_runtime.jsxs)("label", { className: "dsham_settingsSearch", children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconSearchOutline16, {}), (0, react_jsx_runtime.jsx)("input", { type: "search", value: query, onChange: (event) => setQuery(event.target.value), placeholder: t("archives.searchPlaceholder"), "aria-label": t("archives.searchPlaceholder") })] }), (0, react_jsx_runtime.jsx)("select", { className: "dsham_settingsFilter", value: project, onChange: (event) => setProject(event.target.value), "aria-label": t("archives.projectFilter"), children: [(0, react_jsx_runtime.jsx)("option", { value: "all", children: t("archives.allProjects") }), groups.map((group) => (0, react_jsx_runtime.jsx)("option", { value: group.title, children: group.title }, group.title))] })] }), groups.length === 0 ? (0, react_jsx_runtime.jsx)("div", { className: "dsham_settingsEmpty", children: t("archives.empty") }) : filteredGroups.length === 0 ? (0, react_jsx_runtime.jsx)("div", { className: "dsham_settingsEmpty", children: t("archives.emptyFiltered") }) : filteredGroups.map((group) => (0, react_jsx_runtime.jsxs)("section", {
					...
					...deleteTarget === null ? {} : { description: deleteTarget.all ? t("archives.deleteAllDesc", { n: deleteAllSessionIds.length }) : t("deleteSession.desc", { name: displayTitle(deleteTarget, t) }) },
		...
		*/
		...
		const zh = {
			...
			"archives.deleteAllDesc": "将永久删除全部 {n} 个已归档聊天及其子会话和记录，此操作不可恢复。",
			"archives.deleteAllPending": "正在删除已归档聊天…",
			"deleteSession.title": "删除会话",
			"deleteSession.desc": "将永久删除会话“{name}”及其子代理和全部记录（对话内容、统计、缓存），此操作不可恢复。",
		...
		const en = {
			...
			"archives.deleteAllDesc": "This permanently deletes all {n} archived chats, their child sessions, and their records. This cannot be undone.",
			"archives.deleteAllPending": "Deleting archived chats…",
			"deleteSession.title": "Delete session",
			"deleteSession.desc": "This permanently deletes session “{name}”, its child agents, and all of its records (conversation, stats, cache). This cannot be undone.",
