window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* 仅供 __test 使用：展开已归档 id 及其仍在目录中的 subagent 子会话，先子后父。运行时删除只提交根会话或归档集合。
		*/
		...
		*/
		...
		function ArchivedSessionsSection({ sessionStore, workspaceStore, unarchiveSession, deleteSession, t }) {
			...
			const groups = (0, react.useMemo)(() => {
				// 列表仍隐藏 subagent；删除全部只提交列表里可见的已归档会话，子代理由服务端级联。
			...
			const deleteAllSessionIds = (0, react.useMemo)(() => groups.flatMap((group) => group.sessions.map((session) => session.id)), [groups]);
			...
			const confirmDelete = async () => {
				...
				setError(null);
				try {
					for (const id of deleteTarget.all ? deleteAllSessionIds : [deleteTarget.id]) {
						try {
							await deleteSession(id);
						} catch (reason) {
							if (deleteTarget.all && isUnknownSessionError(reason)) continue;
							throw reason;
						}
					}
					setDeleteTarget(null);
