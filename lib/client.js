	...
	factory: (require) => {
				...
				(async () => {
					for (const id of ids) {
						try {
							...
							const message = reason instanceof Error ? reason.message : String(reason);
							if (id !== rootId && isUnknownSessionError(reason)) continue;
							throw reason;
						}
					}
				})().then(() => {
				...
				try {
					for (const id of deleteTarget.all ? deleteAllSessionIds : collectSessionAndDescendantIds(deleteTarget.id, sessions.byId)) {
						try {
							...
							const message = reason instanceof Error ? reason.message : String(reason);
							if ((deleteTarget.all || id !== deleteTarget.id) && isUnknownSessionError(reason)) continue;
							throw reason;
						}
					}
					setDeleteTarget(null);
		...
		*/
		...
		const zh = {
			...
			"archives.deleteAllDesc": "将永久删除全部 {n} 个已归档聊天及其子代理和记录，此操作不可恢复。",
		...
		const en = {
			...
			"archives.deleteAllDesc": "This permanently deletes all {n} archived chats, their child agents, and their records. This cannot be undone.",
