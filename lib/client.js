        ...
        const load = async () => {
          ...
          for (let start = 0; start < ids.length && active; start += 20) {
            ...
            try {
              ...
              items2.push(...batch.map((sessionId) => found.get(sessionId) ?? { sessionId, turnCount: null, path: null, error: "", errorKey: "details.notReturned" }));
            } catch (error) {
              items2.push(...batch.map((sessionId) => ({ sessionId, turnCount: null, path: null, error: String(error?.message ?? error) })));
            }
            if (active) setState({ key, items: [...items2], pending: start + 20 < ids.length });
          }
        };
        load();
        return () => {
