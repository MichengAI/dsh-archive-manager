window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* “全部删除”的提交集合：直接取归档全集，仅排除摘要已知为 subagent 的
		* id（其删除由服务端级联）。不依赖摘要是否已加载，投影未就绪的会话
		* 不会被静默漏删。
		* @param archivedSessionIds - 注册表全局归档集合。
		* @param byId - 会话摘要表（可能不完整）。
		* @returns 去重后的待删除 id 列表。
		*/
		function deriveArchivedDeleteAllIds(archivedSessionIds, byId) {
			const seen = /* @__PURE__ */ new Set();
			const ids = [];
			for (const id of archivedSessionIds ?? []) {
				if (typeof id !== "string" || id.length === 0 || seen.has(id)) continue;
				seen.add(id);
				if (byId?.[id]?.origin === "subagent") continue;
				ids.push(id);
			}
			return ids;
		}
		...
		function ArchivedSessionsSection({ sessionStore, workspaceStore, unarchiveSession, deleteSession, t }) {
			...
			const filteredGroups = (0, react.useMemo)(() => {
				...
				return groups.filter((group) => project === "all" || project === group.key).map((group) => ({
					...group,
			...
			const deleteAllSessionIds = (0, react.useMemo)(() => deriveArchivedDeleteAllIds(workspaceState.archivedSessionIds, sessions.byId), [workspaceState.archivedSessionIds, sessions.byId]);
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				}, group.key)), error !== null && (0, react_jsx_runtime.jsx)("div", { className: "dsham_settingsError", role: "alert", children: error }), (0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
					open: deleteTarget !== null,
					onClose: closeDelete,
					closeLabel: t("close"),
