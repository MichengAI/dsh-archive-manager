...
var ArchiveWorkspaceRegistry = class extends WorkspaceRegistry {
    ...
    for (const sessionId of sessionIds) {
      ...
      try {
        ...
        try {
          ...
          item.error = "\u8DEF\u5F84\u4E0D\u53EF\u7528\uFF1A" + String(error?.message ?? error).slice(0, 350);
        }
  ...
  async diagnoseSession(input) {
    ...
    try {
      ...
      return { ...result, code: "blocked", advice: "\u6682\u4E0D\u80FD\u81EA\u52A8\u4FEE\u590D\uFF1A" + String(error?.message ?? error).slice(0, 850) };
    }
  }
  ...
  async searchSessionContent(input) {
    ...
    for (const sessionId of sessionIds) {
      try {
        ...
        failures.push({ sessionId, message: String(error?.message ?? error).slice(0, 500) });
      }
    }
