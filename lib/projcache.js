...
/**
 * Archive Manager owns a separate cache domain so DSH alpha.2 never asks the
 * JSON backend to turn a logical IM id such as `im:qq:dm:...` into a Windows
 * filename. The public Session id remains unchanged; only the storage key is
 * a fixed-length SHA-256 token and the original id is carried in the value.
 *
 * On rc.2 the runtime has no per-record layout, so the same domain remains a
 * single JSON file. After that user upgrades to alpha.2, every already-safe
 * hashed key can be migrated by the official backend without hitting the
 * legacy colon bug.
 */
...
async function importMissingRecords(ctx, target, records, label) {
	...
	for (const [sessionId, record] of records) {
		...
		try {
			await target.put(sessionId, record);
			imported += 1;
		} catch (error) {
...
/**
 * Seed the compatibility domain without opening alpha.2's unsafe per-record
 * source while the v3 whole-file cache contains rows. An empty v3 source may
 * fall through to the current source; source-open failures stay fail-soft.
 * Imports are add-only: a newer compatibility-domain checkpoint always wins,
 * and a partially completed import resumes on the next startup.
 */
async function importPreviousProjectionCache(ctx, target) {
	...
	if (!legacy.opened) return 0;
	if (legacy.records.length > 0) return importMissingRecords(ctx, target, legacy.records, "legacy v3");
	...
	if (sameSpec) return 0;
	const current = await readSourceRecords(ctx, projectionCacheDomainSpec, "current");
	if (!current.opened) return 0;
	return importMissingRecords(ctx, target, current.records, "current");
}
 ...
 * `sessionProjectionCache` 与 fail-soft 写路径，但使用独立、路径安全的
 * `session_projcache_archive_manager` 兼容域，为归档管理的
