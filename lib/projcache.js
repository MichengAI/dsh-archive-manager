...
async function readLegacySafeRecords(ctx) {
	const source = await readSourceRecords(ctx, legacySafeProjectionCacheDomainSpec, "archive-manager v1");
	const records = [];
	for (const [physicalKey, stored] of source.records) {
		if (stored === null || typeof stored !== "object" || typeof stored.sessionId !== "string") {
			ctx.logger.warn(`archive-manager projcache: archive-manager v1 row "${physicalKey}" import skipped because its session id is invalid`);
			continue;
		}
		if (projectionCacheStorageKey(stored.sessionId) !== physicalKey) {
			ctx.logger.warn(`archive-manager projcache: archive-manager v1 row "${physicalKey}" import skipped because its storage key does not match the session id`);
			continue;
		}
...
/**
 * 合并归档管理器 v1、官方新版逐会话与旧版整文件缓存。目标域记录最高优先，
 * 其次是归档管理器 v1 和官方新版；任一来源失败时仍继续尝试另一个来源。
 */
async function importPreviousProjectionCache(ctx, target) {
	const legacySafe = await readLegacySafeRecords(ctx);
